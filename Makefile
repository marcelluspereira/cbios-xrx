...
PACKAGE_FULL:=$(PACKAGE_NAME)-$(VERSION)a
...
# If needed override location of ZDS command line tools.
ZDS_BIN?=C:/Program Files (x86)/ZiLOG/ZDS_3.68/bin
ZMA?=$(ZDS_BIN)/zma.exe
ZLD?=$(ZDS_BIN)/Zld.exe
ZDS_ASMDIR=derived/zdsasm
ZDS_PREPROCESSED:=$(patsubst src/%.asm,$(ZDS_ASMDIR)/%.asm,$(wildcard src/*.asm))
...
ifeq ($(Z80_ASSEMBLER),zds)
	@mkdir -p $(ZDS_ASMDIR)
	@mkdir -p derived/obj
	@mkdir -p derived/hex
	@"$(ZMA)" -pZ380 -I$(ZDS_ASMDIR) -Isrc -Iderived/asm \
		-dZDS_BUILD=1 \
		-l$(@:derived/bin/%.rom=derived/lst/%.lst) \
		-o$(@:derived/bin/%.rom=derived/obj/%.o) $(<:vdep/%=$(ZDS_ASMDIR)/%)
	@grep -q "ZMA-E" $(@:derived/bin/%.rom=derived/lst/%.lst) && exit 1 || true
	@"$(ZLD)" -A -a -o $(@:derived/bin/%.rom=derived/hex/%.hex) \
		$(@:derived/bin/%.rom=derived/obj/%.o)
	@ZDS_BASE=0x4000 ; ZDS_SIZE=16384 ; \
	if echo "$*" | grep -q '^main_' ; then ZDS_BASE=0 ; ZDS_SIZE=32768 ; fi ; \
	if echo "$*" | grep -q '^logo_' ; then ZDS_BASE=0x8000 ; fi ; \
	if [ "$*" = "sub" ] ; then ZDS_BASE=0 ; fi ; \
	python tools/intelhex_to_bin.py \
		--base $$ZDS_BASE --size $$ZDS_SIZE \
		$(@:derived/bin/%.rom=derived/hex/%.hex) $@
endif

ifeq ($(Z80_ASSEMBLER),zds)
$(ROMS_FULLPATH): $(ZDS_PREPROCESSED)
endif
...
ifeq ($(Z80_ASSEMBLER),zds)
$(ZDS_ASMDIR)/%.asm: src/%.asm
	@echo "Preprocessing: $<"
	@powershell -NoProfile -ExecutionPolicy Bypass \
		-File tools/preprocess_zds.ps1 \
		-SourceDir src \
		-OutputDir $(ZDS_ASMDIR) \
